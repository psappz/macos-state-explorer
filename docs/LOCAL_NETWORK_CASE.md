...
Artifact metadata includes source artifact metadata, source SHA256, generated artifact SHA256, removed object refs, UID rewrite count, array change count, dictionary change count, simulation verdict, validation result, and reparse status. After writing the offline plist, the command reparses it immediately and fails closed if parsing fails. The CLI writes deterministic sidecar metadata beside the artifact as `ARTIFACT_STEM.json`; for example `networkextension-repair-artifact.plist` produces `networkextension-repair-artifact.json`.
...
Apply output records `final_status`, `dry_run`, `mutation_performed`, `backup_created`, `blockers`, `blocker_details`, expected and actual source hashes, generated artifact hash, artifact path, target path, metadata status, preflight status, whether the only remaining blocker is confirmation, preflight checks, post-apply validation commands, rollback commands, and final status. Blocker details are stable JSON objects. `metadata_missing` lists missing sidecar files and missing fields exactly. `source_sha256_mismatch` includes the expected source SHA256 from metadata, the actual current target SHA256, the target path, the artifact path, and a best-effort relationship such as `same_source_path_but_hash_changed`, `different_source_path`, or `missing_source_metadata`. `system_settings_running` explains that System Settings/System Preferences must be closed because it can read/write or cache the same preference state. `missing_explicit_confirmation` reports the exact required flag: `--confirm-apply APPLY_NETWORKEXTENSION_REPAIR_ARTIFACT`.
...
The validator checks the repaired target in deterministic order: target existence, plist readability, `plutil` linting, NSKeyedArchiver structure, object-graph reconstruction, graph consistency, broken/dangling UID references, malformed arrays/dictionaries, duplicate object-index impossibility, serialization round-trip consistency, object-graph analysis, repair-candidate analysis, candidate-validation analysis, zero remaining removable repair candidates, full semantic comparison with the generated repair artifact, repair-success validation, and summary generation.

The final repair-success validation is deliberately narrower than full-archive equivalence. macOS may regenerate unrelated NSKeyedArchiver objects after installation, so complete decoded archive equality is diagnostic only. A repair is considered effective when archive integrity passes, graph consistency passes, serialization remains valid, no removed/stale NetworkExtension repair target reappears, no removed UID is referenced, no validation candidate remains, and the repaired graph remains repair-relevant equivalent for the NetworkExtension objects touched by the generated artifact. Unrelated Apple-regenerated archive differences are classified as `unrelated_semantic_difference`; only `repair_relevant_difference` fails validation.

JSON and text output include every validation stage with PASS/FAIL, reason, duration, expected and observed values, generated statistics, object count, UID count, UID rewrites, array count, dictionary count, repair candidates remaining, validation candidates remaining, graph consistency, archive integrity, bytewise SHA256 comparison, full semantic equivalence, repair-relevant semantic equivalence, Apple-regenerated unrelated archive object detection, unrelated semantic difference count, repair-relevant semantic difference count, serialization-difference explanation, and an overall verdict: `VALIDATION_PASSED_REPAIR_EFFECTIVE`, `VALIDATION_FAILED`, or `VALIDATION_INCONCLUSIVE`. SHA256 mismatch and full semantic mismatch are diagnostic only when repair-relevant validation passes.

On failure, the command reports the exact failed stage plus expected value, observed value, and a recommended rollback action. It does not perform rollback automatically. If a repair-relevant difference exists, the failure stage is `repair_relevant_semantic_difference` and the explanation states why the repair is not validated as effective.

Support bundles include `networkextension-apply-validation.json` and `networkextension-apply-validation.txt`; local-network reports include `networkextension_apply_validation_summary`. Bundle diffs include `NetworkExtension Apply Validation Diff` with validation-result changes, candidate-count deltas, target/artifact hash changes, full semantic equivalence, repair-relevant semantic equivalence, unrelated/relevant difference deltas, and graph-consistency changes.
