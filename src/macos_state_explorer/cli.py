...
def _bundle_networkextension_apply_validation_diff(before_report: dict[str, Any], after_report: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "repair_actually_successful_before": bool(before.get("repair_actually_successful", False)),
        "repair_actually_successful_after": bool(after.get("repair_actually_successful", False)),
        "repair_relevant_semantic_equivalence_before": bool(before.get("repair_relevant_semantic_equivalence", False)),
        "repair_relevant_semantic_equivalence_after": bool(after.get("repair_relevant_semantic_equivalence", False)),
        "apple_regenerated_unrelated_archive_objects_before": bool(before.get("apple_regenerated_unrelated_archive_objects", False)),
        "apple_regenerated_unrelated_archive_objects_after": bool(after.get("apple_regenerated_unrelated_archive_objects", False)),
        "unrelated_semantic_differences_delta": int(after.get("unrelated_semantic_differences", 0)) - int(before.get("unrelated_semantic_differences", 0)),
        "repair_relevant_semantic_differences_delta": int(after.get("repair_relevant_semantic_differences", 0)) - int(before.get("repair_relevant_semantic_differences", 0)),
        "blocking_repair_relevant_semantic_differences_delta": int(after.get("blocking_repair_relevant_semantic_differences", 0)) - int(before.get("blocking_repair_relevant_semantic_differences", 0)),
        "non_blocking_semantic_differences_delta": int(after.get("non_blocking_semantic_differences", 0)) - int(before.get("non_blocking_semantic_differences", 0)),
...
def _render_bundle_diff(diff: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Repair actually successful: {str(networkextension_apply_validation.get('repair_actually_successful_before', False)).lower()} → {str(networkextension_apply_validation.get('repair_actually_successful_after', False)).lower()}",
        f"- Full semantic equivalence: {str(networkextension_apply_validation.get('semantic_equivalence_before', False)).lower()} → {str(networkextension_apply_validation.get('semantic_equivalence_after', False)).lower()}",
        f"- Repair-relevant semantic equivalence: {str(networkextension_apply_validation.get('repair_relevant_semantic_equivalence_before', False)).lower()} → {str(networkextension_apply_validation.get('repair_relevant_semantic_equivalence_after', False)).lower()}",
        f"- Apple regenerated unrelated archive objects: {str(networkextension_apply_validation.get('apple_regenerated_unrelated_archive_objects_before', False)).lower()} → {str(networkextension_apply_validation.get('apple_regenerated_unrelated_archive_objects_after', False)).lower()}",
        f"- Unrelated semantic differences: {networkextension_apply_validation.get('unrelated_semantic_differences_delta', 0):+d}",
        ...
        f"- Blocking repair-relevant semantic differences: {networkextension_apply_validation.get('blocking_repair_relevant_semantic_differences_delta', 0):+d}",
        f"- Non-blocking semantic drift differences: {networkextension_apply_validation.get('non_blocking_semantic_differences_delta', 0):+d}",
        ...
        "- Full semantic mismatch diagnostic-only without blocking diffs.",
